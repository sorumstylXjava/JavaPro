...
object GameListManager {
    private const val FILE_NAME = "gamelist.txt"

    private fun getFile(context: Context): File {
        return File(context.filesDir, FILE_NAME)
    }

    fun getGameList(context: Context): List<String> {
        val file = getFile(context)
        // Jika file sudah ada dan tidak kosong, baca isinya
        if (file.exists() && file.length() > 0) {
            return file.readLines().map { it.trim() }.filter { it.isNotBlank() }
        } else {
            // Jika file belum ada atau kosong, tulis daftar LENGKAP ini
            val allGames = getDefaultGameList()
            file.writeText(allGames.joinToString("\n"))
            return allGames
        }
    }

    fun addGame(context: Context, pkg: String) {
        val list = getGameList(context).toMutableList()
        if (!list.contains(pkg)) {
            list.add(pkg)
            getFile(context).writeText(list.joinToString("\n"))
        }
    }

    fun removeGame(context: Context, pkg: String) {
        val list = getGameList(context).toMutableList()
        if (list.remove(pkg)) {
            getFile(context).writeText(list.joinToString("\n"))
        }
    ...
    // Daftar Lengkap dari requestmu
    private fun getDefaultGameList(): List<String> {
        return listOf(
            "Nekootan.kfkj",
